CREATE DATABASE  IF NOT EXISTS `constructoras_coder_house` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
...
--
-- Temporary view structure for view `personal_no_asignado`
--

DROP TABLE IF EXISTS `personal_no_asignado`;
/*!50001 DROP VIEW IF EXISTS `personal_no_asignado`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `personal_no_asignado` AS SELECT 
 1 AS `id_personal`,
 1 AS `nombre`,
 1 AS `apellido`,
 1 AS `rol`,
...
--
-- Final view structure for view `personal_no_asignado`
--
...
/*!50001 VIEW `personal_no_asignado` AS select `p`.`id_personal` AS `id_personal`,`p`.`nombre` AS `nombre`,`p`.`apellido` AS `apellido`,`p`.`rol` AS `rol`,`p`.`categoria` AS `categoria` from (`personal` `p` left join `asignacion` `a` on(((`p`.`id_personal` = `a`.`id_personal`) and (`a`.`fecha_fin` is null)))) where (`a`.`id_personal` is null) */;
...
-- Dump completed on 2026-01-16 16:58:40
